...
public protocol PageControlDelegate {
    func pageControl(_ pageControl: PageControl, didPressedOn button: UIButton)
    func pageControl(_ pageControl: PageControl, changeProgress to: Int)
}
...
public class PageControl: UIControl {
    ...
    public var elementWidth: CGFloat = 30 {
        didSet {
    ...
    public var inactiveColor: UIColor = UIColor(red: 255/255, green: 255/255, blue: 255/255, alpha: 0.3) {
        didSet {
            self.setNeedsLayout()
        }
    }
    ...
    public var fillWidthAutomatically: Bool = false {
        didSet {
            if (self.fillWidthAutomatically) {
                let widthPlusSpacing = self.bounds.width - (CGFloat(self.pages - 1) * self.spacing)
                self.elementWidth = (widthPlusSpacing / CGFloat(self.pages))
